...
// ya que es ilegal que el motor funcione sin pedalear, aunque solo lo haga por 
// un segundo y medio, para activarlo hay qu encender la bici con el freno pulsado.
...
void setup() {
  ...
  // Nuevo modo peques, sin niveles y velocidad m�xima, fijada por defecto en 15 kmh
  // puedes fijar otra velocidad cambiando el valor de v_crucero seg�n tablas de arriba
  ...
  if (p_frenadas > 5 && p_frenadas < 15){
    ...
    retardo_aceleracion = 7;  // alargamos el progresivo
    v_crucero = 2.80;  //2.80=15 Kmh, cambiar valor para otra velocidad maxima
    modo_crucero = false;  // Anulamos los niveles con toques de freno
    retardo_paro_motor = 0.25;  // valor por defecto
    desacelera_al_parar_pedal = true;  // valor por defecto
    ayuda_salida_cuesta = false;  // Anulamos la ayuda al arranque
    p_frenadas = 0;
  }
  // Modo alternativo al pedalear aumenta velocidad y al parar pedales disminuye, no tiene niveles
